...
begin
  ...
  if p_owner='automatic_release' and not exists(
    select 1
  ...
  if p_owner='customer_cancel' then
    if not exists(
      ...
      where id=p_request_id
        and (
          status='open'
          or (status='in_progress' and coalesce(job_stage,'')<>'working')
        )
    ) then
    ...
    if exists(
      ...
      where request_id=p_request_id and payment_status='paid'
    ) then
...
begin
  ...
  if tg_table_name='change_orders' then
    ...
    -- The customer_cancel owner may only project an already-settled cancellation
    -- into the terminal reassignment state. No financial/content fields may change.
    if tg_op='UPDATE'
      and coalesce(auth.role(),'')='service_role'
      and v_resolution.owner='customer_cancel'
      and v_old->>'status' in ('available','pending_replacement','applied','cancelled')
      and v_new->>'status'='cancelled'
      and (v_new - array['status','updated_at'])=(v_old - array['status','updated_at'])
      and exists(select 1 from public.service_requests where id=v_id and status='cancelled')
      and not public.co_has_unresolved_payment(v_id)
      and not exists(select 1 from public.change_orders where request_id=v_id and payment_status='paid')
      and not exists(select 1 from public.job_financial_steps where request_id=v_id and receipt is null)
    then
      null;
    elsif v_resolution.request_id is not null or public.co_has_unresolved_payment(v_id) or
      exists(select 1 from public.change_orders where request_id=v_id and payment_status='paid') then
