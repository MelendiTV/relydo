...
async function recoverCancelledCustomerReassignments(requestId: string) {
  // A cancelled job must never create a new financial reservation here.
  // Calling the RPC directly is only a probe: on a cancelled job, success means
  // the exact customer_cancel reservation already existed and is safe to resume.
  const { data: existingResolution, error: existingResolutionError } =
    await supabaseAdmin.rpc("reserve_job_financial_resolution", {
      p_request_id: requestId,
      p_owner: "customer_cancel",
      p_decision: {},
    });

  if (existingResolutionError) {
    const message = String(existingResolutionError.message || "");

    // No matching customer_cancel reservation exists. This can be a historical
    // cancellation or a job owned by another reconciliation path; do not mutate
    // payment_reassignments from this recovery branch.
    const noCustomerCancelReservation = [
      "JOB_NOT_CANCELLABLE_BY_CUSTOMER",
      "FINANCIAL_OWNER_CONFLICT",
      "CHANGE_ORDER_RECONCILIATION_REQUIRED",
      "PAID_CHANGE_ORDER_REQUIRES_ADMIN",
      "CLAIM_REQUIRES_ADMIN_RECONCILIATION",
    ].some((code) => message.includes(code));

    if (noCustomerCancelReservation) {
      return { attempted: false, closed: 0 };
    }

    throw new Error(
      `No pudimos comprobar la reserva financiera de la cancelación: ${message}`
    );
  }

  if (!existingResolution?.reserved) {
    return { attempted: false, closed: 0 };
  }

  // Re-run the normal guard recovery so any durable financial step that already
  // has an observable Stripe effect gets its receipt recorded before finalizing.
  await reserveJobResolution(
    supabaseAdmin,
    requestId,
    "customer_cancel",
    {},
  ...
  const { data: pendingReassignments, error: pendingReassignmentsError } =
    await supabaseAdmin
      .from("payment_reassignments")
      .select("id,status")
      .eq("request_id", requestId)
      .in("status", ["available", "pending_replacement", "applied"]);

  if (pendingReassignmentsError) {
    throw new Error(
      `La solicitud ya está cancelada, pero no pudimos consultar sus reasignaciones pendientes: ${pendingReassignmentsError.message}`
    );
  }
  ...
  for (const reassignment of pendingReassignments || []) {
    const { error: closeReassignmentError } = await supabaseAdmin
      .from("payment_reassignments")
      .update({
        status: "cancelled",
      ...
      .eq("status", reassignment.status);

    if (closeReassignmentError) {
      throw new Error(
        `La solicitud ya está cancelada, pero RELYDO todavía no pudo cerrar la reasignación ${reassignment.id}: ${closeReassignmentError.message}`
      );
    }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (serviceRequest.status === "cancelled") {
      ...
      const reassignmentRecovery =
        await recoverCancelledCustomerReassignments(requestId);
      ...
      return NextResponse.json({
        ...
        recoveredReassignments: reassignmentRecovery.closed,
